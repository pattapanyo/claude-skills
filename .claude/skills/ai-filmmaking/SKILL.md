...
**3. Always include the biometric landmark grid on face shots — unless the user explicitly says to exclude it.** Every close-up in the bottom row must carry a subtle, very thin semi-transparent 6×6 white grid overlay perfectly aligned over the face. This breaks the face into biometric landmark zones, making it far easier to match facial geometry across downstream shots and generations. It is on by default. The user must say "no grid" or "exclude the overlay" to suppress it.
...
- **Omitting the biometric grid on character face shots.** The 6×6 overlay is on by default — only drop it when the user explicitly says so.
